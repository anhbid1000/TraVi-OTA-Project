...
type LoyaltySummaryCardsProps = {
  summary: LoyaltySummary
  availableVoucherCount: number
}

export function LoyaltySummaryCards({ summary, availableVoucherCount }: LoyaltySummaryCardsProps) {
  return (
    <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
      ...
      <article className="rounded-xl border border-outline-variant/40 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-2xl font-bold text-primary">{availableVoucherCount}</p>
      </article>
    </section>
  )
}
