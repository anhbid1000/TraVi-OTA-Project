...
import { ExchangeVoucherPanel } from './ExchangeVoucherPanel'
...
import { VoucherWalletPanel } from './VoucherWalletPanel'
...
export default function LoyaltyDashboardPage() {
  ...
  return (
    <CustomerFeedbackLayout
      active="loyalty"
      title="Loyalty & ví voucher"
      subtitle="Theo dõi điểm thưởng, hạng thành viên và đổi ưu đãi bằng điểm."
    >
      ...
      {!loading && summary && (
        <div className="space-y-6">
          <LoyaltySummaryCards summary={summary} availableVoucherCount={walletByStatus.available.length} />
          <TierProgressPanel progress={progress} />

          <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <PointHistoryPanel history={history} />
            <VoucherWalletPanel wallet={wallet} walletByStatus={walletByStatus} />
          </section>

          <ExchangeVoucherPanel
            exchangeables={exchangeables}
            currentPoints={summary.currentPoints ?? 0}
            exchangingVoucherId={exchangingVoucherId}
            onExchange={(voucherId) => void handleExchange(voucherId)}
          />
        </div>
      )}
    </CustomerFeedbackLayout>
