...
function getRoomComboTitle(combo: RoomCombinationOption, index: number) {
  const mainTypes = combo.items
    .map((item) => (item.quantity > 1 ? `${item.quantity}x ${item.roomName}` : item.roomName))
    .join(' + ');
...
export function HotelDetailPage() {
  ...
  const detailParams = useMemo(() => ({ checkIn, checkOut, guests }), [checkIn, checkOut, guests]);

  const { data: hotel, loading, error, refetch } = useHotelDetail(id, detailParams);
  ...
  const selectedCombo =
    selectionType === 'combo' && selectedComboIndex !== null
  ...
  const totalPrice = selectedCombo
    ...
    : selectedRoomEntries.reduce(
        (sum, entry) => sum + entry.room.pricePerNight * entry.quantity * nights,
        0
      );
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-8 pb-44 md:px-12">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <nav className="text-sm text-on-surface-variant">
            ...
            <Link to="/hotels" className="cursor-pointer hover:text-primary">
              Khách sạn
            </Link>
            <span className="mx-2">/</span>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          <section className="space-y-10 lg:col-span-8">
            ...
            {roomCombinationOptions.length > 0 && (
              <section>
                <h2 className="mb-5 font-display text-3xl font-bold text-primary">
                  Gợi ý phòng cho {guests} khách
                </h2>
              ...
              {availableRooms.length === 0 ? (
                <div className="rounded-2xl border border-outline-variant/30 bg-white p-8 text-center shadow-sm">
                  ...
                  <p className="mt-2 text-sm text-on-surface-variant">
                    Bạn có thể thử đổi ngày hoặc số khách.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {availableRooms.map((room) => {
                    ...
                    return (
                      <SelectableItemCard
                        ...
                        statusLabel={
                          isSelected
                            ? `Đã chọn ${selectedQuantityForRoom}`
                            : `Còn ${room.availableQuantity} phòng`
                        }
                        statusTone={
                          isSelected
                            ? 'success'
                            : room.availableQuantity <= 2
                              ? 'warning'
                              : 'success'
                        }
                        actionLabel={
                          selectedQuantityForRoom >= room.availableQuantity
                            ? 'Đã chọn tối đa'
                            : isSelected
                              ? 'Thêm phòng'
                              : 'Chọn phòng'
              ...
              <article className="rounded-2xl border border-outline-variant/30 bg-white p-6 shadow-sm">
                ...
                <div className="space-y-2 text-sm text-on-surface-variant">
                  <p className="flex items-center gap-2">
                    <Expand size={14} /> Số đêm: {nights}
                  </p>
                  <p className="flex items-center gap-2">
                    <Users size={14} /> Số khách: {guests}
                  </p>
                  <p className="flex items-center gap-2">
                    <BedDouble size={14} /> Phòng chọn: {selectedOptionLabel || 'Chưa chọn'}
                  </p>
                </div>
              </article>
            </div>
