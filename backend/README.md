...
| Route | Quyền | Mô tả |
|---|---|---|
| `/api/auth/**` | Public | Đăng ký, Login, Logout, Verify OTP, Refresh |
...
- Giới hạn **5 requests/phút** cho `/api/auth/login`
...
| PUT | `/api/auth/verify-email` | ❌ | - | ✅ | Xác minh OTP và kích hoạt tài khoản |
| POST | `/api/auth/login` | ❌ | - | ✅ | Đăng nhập và lấy JWT (Access + Refresh) |
| POST | `/api/auth/logout` | ✅ | - | ✅ | Đăng xuất (blacklist token) |
| POST | `/api/auth/refresh` | ❌ | - | ✅ | Làm mới Access Token bằng Refresh Token |
| GET/POST/PUT/DELETE | `/api/hotels/public/**` | ❌ | - | 🔄 | Tìm kiếm khách sạn công khai (Sprint 2) |
| GET/POST/PUT/DELETE | `/api/restaurants/public/**` | ❌ | - | 🔄 | Tìm kiếm nhà hàng công khai (Sprint 2) |
| GET/POST/PUT/DELETE | `/api/admin/**` | ✅ | QUAN_TRI_VIEN | 🔄 | Dashboard quản trị (Sprint 2) |
| GET/POST/PUT/DELETE | `/api/partner/**` | ✅ | DOI_TAC | 🔄 | Dashboard đối tác (Sprint 2) |
...
- `loadUserByUsername(username)` → Load user từ DB (override `UserDetailsService`)
...
curl -X PUT http://localhost:8080/api/auth/verify-email \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "confirmOTP": "123456"
